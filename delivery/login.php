...
require 'includes/cnx.php';
 if($_SERVER['REQUEST_METHOD'] == "POST")
 {
    if(isset($_POST['login']))
    {
        ...
        $stmt = $cnx->prepare("SELECT * from livreur WHERE Email = '$email' AND MDP = '$password'");
        $stmt->execute();
        $count = $stmt->rowCount();
        $row = $stmt->fetch();
        if($count > 0)
        {
            $_SESSION['delivery'] = $row['CIN'];
...
<body>
    <div class="login-page">
        <div class="container">
            <h1 class="text-center login-title"><span data-class ="login" class="selected-login">Delivery Login</span></h1>
            <!-- Start Login -->
